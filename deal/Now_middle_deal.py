...
data = openpyxl.load_workbook('ne.xlsx')
...
for i in range (2 ,Sheet_Row_Num+1):
    if sheet_data.cell(row=i , column=1).value!=sheet_data.cell(row=i+1 , column=1).value:
        ...
        Aount = Aount + sheet_data.cell(row=i , column=11).value
        ...
        Aount = sheet_data.cell(row= i, column=10).value + sheet_data.cell(row= i + 1, column=10).value
