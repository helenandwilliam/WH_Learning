...
data = openpyxl.load_workbook('Mid_Deal_Table1.xlsx')
...
table.cell(row= 1, column= 3).value= '总营业额'
...
for i in range (2 ,Sheet_Row_Num+1):
	if sheet_data.cell(row=i , column=1).value!=sheet_data.cell(row=i+1 , column=1).value:
		...
		Aount = Aount + sheet_data.cell(row=i , column=9).value
